...
sub tryMoves {
    ...
    # The BFS is achieved by throwing new sequences onto the end of a stack. You then keep pulling sequences
    ...
    # Lets seed the queue with no previous moves so it will start off with the starting grid
    ...
    while ($sequence = shift @moveQueue) { 
        ...
        # Lets see what are the moves that we can do from here based on the current grid
        ...
        # Try each of the possible moves, if it solves the puzzle we are done. Otherwise take the new 
        # sequnce of moves and throw it on the stack, unless we have seen the grid patter before
        ...
        foreach $step (@moves) {
            ...
            if (&isItSolved($newGrid)) {
                ...
                # That did not create a solution, before we push this sequence for further sequencing we want to see if this
                # pattern has been encountered before. If it has there is no point trying more variations as we already
                # have a sequence that gets here and it might have been shorter, thanks to our BFS. This turns the program
                # from doing a massive amount of work to a very efficient tiny set of branches. 
                
                if (!&seen($newGrid)) {
                    # Um, looks like it was not solved and its a new path, lets throw this grid on the queue for another 
                    # attempt. You never know it might lead to the final solution
